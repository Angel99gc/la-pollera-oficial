...
const MenuPage = () => {
  ...
  return (
    <>
    <main>
  <section id="FullMenu" className="relative min-h-screen flex items-center justify-center overflow-hidden">
            ...
            <div className="space-y-4">
              {Object.entries(menuData).map(([category, subcategories]) => (
                <div key={category} className="space-y-2">
                  <button onClick={() => setActiveCategory(category)} className={`w-full text-left px-4 py-2 rounded-lg ${activeCategory === category ? "bg-orange-500 text-white" : "text-gray-600 hover:bg-orange-100"}`}>
                    {category.charAt(0).toUpperCase() + category.slice(1)}
                  </button>
                  {activeCategory === category && (
                    <div className="ml-4 space-y-1">
                      {Object.keys(subcategories).map((subCategory) => (
                        <button key={subCategory} onClick={() => setActiveSubCategory(subCategory)} className={`w-full text-left px-4 py-1 rounded-lg text-sm ${activeSubCategory === subCategory ? "bg-orange-200 text-orange-800" : "text-gray-500 hover:bg-orange-50"}`}>
